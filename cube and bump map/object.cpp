...
void object::render()
{
	if(!culled)
	{
		...
		d3ddevice->SetPixelShaderConstant( 5, &D3DXVECTOR4(1,1,1,1), 1 );
...
//set up transforms
		...
		eye_pos.x = view_pos.x;
		eye_pos.y = view_pos.y;
		eye_pos.z = view_pos.z;
		eye_pos.w = 1;
		d3ddevice->SetVertexShaderConstant( 14, &eye_pos, 1 );

//Set light vector and view vector
		D3DXVec4Transform(&light_vec,&light_vec,&temp);
		D3DXVec4Transform(&eye_pos,&eye_pos,&temp);
		D3DXMatrixTranspose( &matworld, &matworld);
		d3ddevice->SetVertexShaderConstant( 6, &matworld, 4 );
...
//set texture
		...
		if(cube_texture != NULL)
			d3ddevice->SetTexture(2,cube_texture);

//render object	
		d3ddevice->SetStreamSource( 0, vertex_buffer, vertex_size );
...
{
	if(rotate)	angle += speed;
	if(angle > 6.28) angle -= 6.28f;
}
...
//calc s and t
	...
	{
		float temp;
		temp = (float)sqrt((data[count].sx * data[count].sx) + (data[count].sy * data[count].sy) +
			(data[count].sz * data[count].sz));
		data[count].sx /= temp;
		data[count].sy /= temp;
		data[count].sz /= temp;
		temp = (float)sqrt((data[count].tx * data[count].tx) + (data[count].ty * data[count].ty) +
			(data[count].tz * data[count].tz));
		if(temp > 0.0f)
		{
			data[count].tx /= temp;
			data[count].ty /= temp;
...
void object::SetSpecPower(float p)
{
	...
	float temp;
	unsigned char tempi;
//create specular power lookup texture
...
LPDIRECT3DCUBETEXTURE8 object::GetCubeTexture()
{
	return cube_texture;
}

void object::SetShaderNumber(int arg)
{
	shader_number = arg;
}


void object::SetRotate(bool arg)
{
	rotate = arg;
}

void object::Cull(D3DXVECTOR3 *look_at, D3DXVECTOR3 *vpos)
{
	float temp;
	D3DXVECTOR3 vvec;
	GetPosition(vvec.x,vvec.y,vvec.z);
	vvec = vvec - (*vpos);
	D3DXVec3Normalize(&vvec,&vvec);
	temp = D3DXVec3Dot(&vvec,look_at);
	if(temp > 0.6f)
		culled = false;
	else culled = true;
}
