...
HPTGraphicsEngine::HPTGraphicsEngine(int quality)
{
...
/*		d3d_device->SetSamplerState(0,D3DSAMP_MAXANISOTROPY,8);
	...
	D3DXCreateFont(d3d_device,20,20,700,1,false,ANSI_CHARSET,OUT_DEFAULT_PRECIS,DEFAULT_QUALITY,DEFAULT_PITCH | FF_DONTCARE,"Arial",&d3dfont);
	ChangeGraphicsState(0);
}

HPTGraphicsEngine::~HPTGraphicsEngine()
{
...
//	if(star_pixel_shader != NULL) star_pixel_shader->Release(); 
	...
	if(star_vertex_shader_points != NULL) star_vertex_shader_points->Release(); 
	if(star_pixel_shader_points != NULL) star_pixel_shader_points->Release(); 
...
int HPTGraphicsEngine::LoadHGF(TCHAR *name)
{
...
void HPTGraphicsEngine::Release()
{
	delete this;

}

HPT3DObject* HPTGraphicsEngine::Create3DObject()
{
...
HPTGraphicsEngine& HPTGraphicsEngine::operator<<(char *arg)
{
	d3d_device->SetPixelShader(NULL);
	d3d_device->SetVertexShader(NULL);
	RECT tempr;
...
HPTGraphicsEngine& HPTGraphicsEngine::operator<<(int arg)
{
	return *this;
	
}

HPTGraphicsEngine& HPTGraphicsEngine::operator<<(unsigned int arg)
{
	return *this;
	
}

HPTGraphicsEngine& HPTGraphicsEngine::operator<<(double arg)
{
		char temp[30];
	sprintf(temp,"%f",arg);
	int pos = 0;
...
HPTGraphicsEngine& HPTGraphicsEngine::Position(int x,int y)
{
	text_positionx = x;
	text_positiony = y;
...
void HPTGraphicsEngine::CreateSpriteVertexBuffer()
{
...
void HPTGraphicsEngine::RenderSprite(D3DXMATRIX& world,int texture,D3DXVECTOR4& color)
{
	D3DXMATRIX temp;
...
void HPTGraphicsEngine::RenderSphere(D3DXMATRIX& world,int texture,int texture2,D3DXVECTOR4& color,float np)
{
	D3DXMATRIX temp;
...
void HPTGraphicsEngine::RenderSpherePoints(D3DXMATRIX& world,int texture,int texture2,D3DXVECTOR4& color,float np)
{
	...
	d3d_device->SetVertexShaderConstantF(4,temp,4);
	d3d_device->SetTexture(0,star_texture->front_texture);
	d3d_device->SetTexture(1,star_texture->front_texture);
	d3d_device->SetIndices(sphere_index_buffer);
	d3d_device->DrawIndexedPrimitive(D3DPT_TRIANGLELIST,0,0,sphere_v_size,0,sphere_size);
...
void HPTGraphicsEngine::RenderSpherePoints2(D3DXMATRIX& world,int texture,int texture2,D3DXVECTOR4& color,float np)
{
	star_texture->RenderTexture2();
	D3DXMATRIX temp;
	...
	d3d_device->SetVertexShaderConstantF(30,color,1);
	d3d_device->SetVertexShaderConstantF(0,temp,4);
	d3d_device->SetPixelShaderConstantF(5,D3DXVECTOR4(0.5f,0.5f,0.5f,1.0f),1);
...
void HPTGraphicsEngineInternal::RenderSphere2(D3DXMATRIX& world,int texture,int texture2,D3DXVECTOR4& color,float np)
{
	star_texture->RenderTexture2();
	BeginFrame();
	D3DXMATRIX temp;
	...
	d3d_device->SetVertexShaderConstantF(0,temp,4);
	d3d_device->SetPixelShaderConstantF(5,D3DXVECTOR4(0.5f,0.5f,0.5f,1.0f),1);
	d3d_device->SetPixelShaderConstantF(6,color,1);
...
//	D3DXMatrixMultiply(&temp,&temp,&matview);
	...
	d3d_device->SetVertexShaderConstantF(14,D3DXVECTOR4(0,0,-1,0),1);
...
*/
	...
	/* ffer.push_back((buffer.size()-1)-1-(sphere_cuts+1));
	ibuffer.push_back((buffer.size()-1));
	ibuffer.push_back((buffer.size()-1)-(sphere_cuts+1));
	ibuffer.push_back((buffer.size()-1)-1-(sphere_cuts+1));
	ibuffer.push_back((buffer.size()-1)-1);
...
/*void HPTGraphicsEngineInternal::CreateRingVertexBuffer()
{
	...
	ring_vertex_buffer->Lock(0,0,(void**)&tempv,0);
	memcpy(tempv,&(buffer[0]),buffer.size()*sizeof(SpriteVertex));
...
*/		switch(arg)
		...
		case 1: //star
			...
			break;
		case 2: //star
			d3d_device->SetVertexShader(star_vertex_shader_points);
			d3d_device->SetPixelShader(star_pixel_shader_points);
			d3d_device->SetVertexDeclaration(sprite_vertex_decl);
			d3d_device->SetStreamSource(0,sphere_vertex_buffer,0,sizeof(SpriteVertex));
			d3d_device->SetVertexShaderConstantF(10,D3DXVECTOR4(0.15f,0.15f,0.15f,1.0f),1);
			d3d_device->SetVertexShaderConstantF(20,D3DXVECTOR4(0,0,0,0),1);
...
void HPTGraphicsEngineInternal::LoadShaders()
{
	LPD3DXBUFFER tempvbufer;
	LPD3DXBUFFER emsg;
	if(D3DXAssembleShaderFromFile("sprite vertex shader.txt",NULL,NULL,0,&tempvbufer,&emsg) != D3D_OK)
	{
		MessageBox(NULL,(const char*)emsg->GetBufferPointer(),"error",MB_OK);
	}
	d3d_device->CreateVertexShader((const unsigned long*)tempvbufer->GetBufferPointer(),&sprite_vertex_shader);
...
/*	if(D3DXAssembleShaderFromFile(hptstrcat(path,"planet vertex shader.txt"),NULL,NULL,0,&tempvbufer,&emsg) != D3D_OK)
	...
	d3d_device->CreateVertexShader((const unsigned long*)tempvbufer->GetBufferPointer(),&planet_vertex_shader);
*/
/*	if(D3DXAssembleShaderFromFile("star vertex shader.txt",NULL,NULL,0,&tempvbufer,&emsg) != D3D_OK)
	{
		MessageBox(NULL,(const char*)emsg->GetBufferPointer(),"error",MB_OK);
	}
	d3d_device->CreateVertexShader((const unsigned long*)tempvbufer->GetBufferPointer(),&star_vertex_shader);

	if(D3DXAssembleShaderFromFile("star pixel shader.txt",NULL,NULL,0,&tempvbufer,&emsg) != D3D_OK)
	{
		MessageBox(NULL,(const char*)emsg->GetBufferPointer(),"error",MB_OK);
	}
	d3d_device->CreatePixelShader((const unsigned long*)tempvbufer->GetBufferPointer(),&star_pixel_shader);
...
/*	if(D3DXAssembleShaderFromFile(hptstrcat(path,"planet vertex shader med.txt"),NULL,NULL,0,&tempvbufer,&emsg) != D3D_OK)
	...
	d3d_device->CreatePixelShader((const unsigned long*)tempvbufer->GetBufferPointer(),&planet_pixel_shader_med);
*/
	if(D3DXAssembleShaderFromFile("star vertex shader high.txt",NULL,NULL,0,&tempvbufer,&emsg) != D3D_OK)
	{
		MessageBox(NULL,(const char*)emsg->GetBufferPointer(),"error",MB_OK);
	}
	d3d_device->CreateVertexShader((const unsigned long*)tempvbufer->GetBufferPointer(),&star_vertex_shader_high);

	if(D3DXAssembleShaderFromFile("star pixel shader high.txt",NULL,NULL,0,&tempvbufer,&emsg) != D3D_OK)
	{
		MessageBox(NULL,(const char*)emsg->GetBufferPointer(),"error",MB_OK);
	}
	d3d_device->CreatePixelShader((const unsigned long*)tempvbufer->GetBufferPointer(),&star_pixel_shader_high);

	if(D3DXAssembleShaderFromFile("star vertex shader points.txt",NULL,NULL,0,&tempvbufer,&emsg) != D3D_OK)
	{
		MessageBox(NULL,(const char*)emsg->GetBufferPointer(),"error",MB_OK);
	}
	d3d_device->CreateVertexShader((const unsigned long*)tempvbufer->GetBufferPointer(),&star_vertex_shader_points);

	if(D3DXAssembleShaderFromFile("star pixel shader points.txt",NULL,NULL,0,&tempvbufer,&emsg) != D3D_OK)
	{
		MessageBox(NULL,(const char*)emsg->GetBufferPointer(),"error",MB_OK);
	}
	d3d_device->CreatePixelShader((const unsigned long*)tempvbufer->GetBufferPointer(),&star_pixel_shader_points);
...
/*	HANDLE filehandle = CreateFile("shaders.hsf",GENERIC_READ,0,NULL,OPEN_EXISTING,FILE_ATTRIBUTE_NORMAL,NULL);
	if(filehandle == INVALID_HANDLE_VALUE)
	{
		MessageBox(NULL,"shaders.hsf",_T("could not open shader.hsf file"),MB_OK);
		return;
	}
	HANDLE filemapping = CreateFileMapping(filehandle,NULL,PAGE_READONLY | SEC_COMMIT,0,0,NULL);
	if(filemapping == NULL)
	{
		MessageBox(NULL,"shaders.hsf",_T("could not map requested HSF file"),MB_OK);
		return;
	}
	void *filepointer = MapViewOfFile(filemapping,FILE_MAP_READ,0,0,0);
	if(filepointer == NULL)
	{
		MessageBox(NULL,"shaders.hsf",_T("could not view requested HSF file"),MB_OK);
		return;
	}
...
void HPTGraphicsEngineInternal::ResetStarTextureForFull()
{
	star_texture->ResetForFull();
	star_texture->ResetForFull();
}
