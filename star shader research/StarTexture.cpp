...
void StarTexture::DoHeat()
{
	...
	for(int count = 0;count < (NUM_POINTS/2);count++)
	{
		...
		else
		{
			...
			//if done create new heat point pair
			if(heat_points[count*2].heat <= 0)
			{
				//first point adding heat
				...
				//second point removing same amount of heat at diffrent location
		...
		//amount of heat to add and remove
...
void StarTexture::Reset()
{
	time_to_filter = 1.0f/10.0f;
	heat_added = 0;
	IDirect3DTexture9* temp_texture;
	temp_texture = front_texture;
	front_texture = back_texture;
...
void StarTexture::DoFilter()
{
	...
	d3d_device->SetVertexShaderConstantF(6,D3DXVECTOR4(1.0/width,1.0/height,1.0/(width*2),1.0/(height*2)),1);
	d3d_device->SetPixelShaderConstantF(10,D3DXVECTOR4(FILTER_SPEED*time,1/5.0f,time,time),1);
